  ...
  try {
    ...
    switch (investigation_type) {
      case "timeline_analysis":
        results = await db.selectFrom("security_logs").selectAll().where("ip_address", "=", params.ip).where("created_at", ">=", new Date(params.start_time)).where("created_at", "<=", new Date(params.end_time)).orderBy("created_at", "asc").execute();
        break;
      case "attack_pattern_analysis":
        results = await db.selectFrom("security_logs").select([
          "event_type",
          "severity",
          "ip_address",
          "message",
          "created_at"
        ]).where("message", "like", `%${params.pattern}%`).orderBy("created_at", "desc").limit(100).execute();
        break;
      default:
