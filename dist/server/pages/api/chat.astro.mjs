...
const POST = async ({ request }) => {
  try {
    ...
    const apiKey = "xai-ECbo2VeJaos5b7zNal3gYaOU1Zfn09NvQG3ujzRfjDwSEo0s2pBw3zEQAsx3MQjh9Yst1jONVaRHEJ80";
    console.log("API Key configurada:", !!apiKey);
    if (!apiKey) ;
    let conversationHistoryFromDB = [];
    ...
    if (emailMatch || phoneMatch) {
      try {
        ...
        const previousConversations = await dbOperations.getConversationHistory(null, phone, email);
        if (previousConversations.length > 0) {
          console.log(`📚 Encontradas ${previousConversations.length} conversas anteriores`);
          conversationHistoryFromDB = previousConversations.slice(0, 3).map((conv) => {
            let messages = [];
            try {
            ...
            const historyMessage = {
              role: "system",
              content: `Conversa anterior (${new Date(conv.created_at).toLocaleDateString()}): Cliente ${conv.client_name || "Não informado"}, problema: ${conv.client_problem || "Não especificado"}. Últimas mensagens: ${Array.isArray(messages) ? messages.slice(-2).map((m) => `${m.role}: ${m.content}`).join("; ") : "Nenhuma mensagem disponível"}`
            };
            console.log("🔍 Histórico formatado:", historyMessage);
            return historyMessage;
          });
          console.log("📋 Total de mensagens de histórico:", conversationHistoryFromDB.length);
        }
      } catch (error) {
...
Lembre-se: SEMPRE use técnicas de persuasão, crie urgência genuína, demonstre valor superior e feche assumindo a venda. Seu objetivo é converter 70%+ dos leads qualificados.`
      ...
      console.log("📤 Mensagens enviadas para Grok:", JSON.stringify(messages.slice(-5), null, 2));
