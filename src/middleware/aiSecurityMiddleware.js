...
import { AISecurityEngine } from '../utils/aiSecurity.js';
import { logAIThreat, logBotDetection, logSpamAttempt } from '../utils/securityLogger.js';
...
class AISecurityMiddleware {
  ...
  getClientIP(request) {
    ...
    // Try to get IP from URL or connection info
    const url = new URL(request.url);
    
    // For development, try to get real client IP
    if (url.hostname === 'localhost' || url.hostname === '127.0.0.1') {
      // In development, try to get the actual client IP
      const clientIP = request.headers.get('host');
      if (clientIP && clientIP.includes(':')) {
        return '127.0.0.1'; // Local development IP
      }
    }

    // Try other methods to get IP
    const remoteAddr = request.headers.get('remote-addr') || 
                      request.headers.get('x-remote-addr') ||
                      request.headers.get('connection-remote-addr');
...
export const aiSecurityMiddleware = new AISecurityMiddleware();

// Export middleware function for use in API routes
export async function validateAISecurityMiddleware(request) {
